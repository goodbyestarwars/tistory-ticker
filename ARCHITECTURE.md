...
- **Tistory는 저장소를 pull하지 않는다.** `skin.html`은 Tistory 관리자 스킨 편집기에 수동
  붙여넣기해야 하며, 저장소의 `skin.html` 변경만으로 블로그 HTML이 바뀌지 않는다.
- **GitHub Pages는 정적 자산 서버다.** `master`의 `js/`, `css/`, `data/`, `style.css`를
  Tistory 스킨이 로드한다. 일반 화면 수정은 `master` push 후 Pages 캐시가 갱신되면 반영된다.
- **GitHub Actions는 GAS 자동 배포에만 사용한다.** `gas/` 변경 시 `clasp push`와 기존
  배포 ID 대상 `clasp deploy`를 실행한다. Actions Packages·LFS는 현재 운영 경로가 아니다.
- **VM은 실시간·보호 API와 영속 캐시를 담당한다.** FastAPI가 `goodbyestar.cloud`에서
  REST와 WebSocket을 제공하고, SQLite·JSON 캐시와 백그라운드 수집기를 함께 관리한다.
- **경제 종합뉴스와 실시간 종목판은 시장 선택을 분리한다.** 국내 탭은 국내 데이터,
  미국 탭은 미국·글로벌 데이터를 사용한다. 경제뉴스 WebSocket은 연결당 초기 스냅샷을
  보내고 서버 캐시를 5분 주기로 갱신하며, 속보 UI의 한 건씩 넘김은 프론트에서 5초 간격으로
  수행한다.
...
- 파일명 버저닝 금지 — 같은 URL이 티스토리 HTML에 그대로 박혀 있어서, 새 파일을 만들지 않고
  기존 파일을 계속 덮어쓴다
- 공통 메뉴는 `js/skin-menu.js`가 `#nav-menu-mount`에 렌더링한다. 1차 메뉴는 6개이며
  시장·종목·종목검색은 드롭다운 없이 고정형 2차 메뉴 바를 연다. 관심종목은 메뉴가 아니라
  `js/stock-search-panel.js`가 모든 페이지에 주입하는 우측 고정 드로어로 제공한다.
...
- 뉴스 모멘텀은 `news_momentum.py`(별도 `news_momentum.db` 스키마·이슈 집계·DataLab 저장)와
  ...
  감성별 기사 수와 최근/이전 7일 비교는 사용자 요청 시 계산하지 않고 배치가
  `news_topic_daily`와 `news_topics`에 집계한다. 스키마 버전 마커가 바뀌면 같은 날이라도
  지정 8종목 배치를 한 번 다시 실행하고, 성공 후에만 날짜·스키마 버전 마커를 갱신한다.
...
| Groq API (`openai/gpt-oss-120b`) | AI 요약 전반(종목뉴스 3문장 요약, 오늘 등락 이유 한줄요약, 시황분석, 투자의견 근거, 서브인덱스 해설) | GAS(`callGroq`), 스크립트 속성에 키 저장 |
