...
// 이어달리기(relay) 공용 상수/헬퍼 - scanInvestSignalWorker_/scanChartPatternsWorker_/
// scanPullbackWorker_ 셋 다 같은 패턴 사용: ~5분 일하다 남으면 커서+중간결과를 저장하고
// 1분 뒤 자기 자신을 재예약. 재예약 트리거 이름은 항상 고정 일일 킥오프 트리거와 달라서
// (예: scanChartPatterns(고정) vs scanChartPatternsWorker_(재예약)) 서로 안 건드림.
...
function scanChartPatterns() {
  scanChartPatternsWorker_();
}

function scanChartPatternsWorker_() {
  clearRelayTriggers_('scanChartPatternsWorker_');

  var props = PropertiesService.getScriptProperties();
  var universe = fetchFullUniverse_();

  var cursor = parseInt(props.getProperty('PATTERN_SCAN_CURSOR') || '0', 10);
  var cycleStartedAt = parseInt(props.getProperty('PATTERN_SCAN_CYCLE_STARTED_AT') || '0', 10);
  var isNewCycle = cursor === 0 || !cycleStartedAt;
  if (isNewCycle) cycleStartedAt = Date.now();

  var results = isNewCycle
  ...
  var scanned = isNewCycle ? 0 : parseInt(props.getProperty('PATTERN_SCAN_WIP_SCANNED') || '0', 10);
  ...
  if (i < universe.length && !forceFinish) {
    props.setProperties({
      PATTERN_SCAN_CURSOR: String(i),
      PATTERN_SCAN_CYCLE_STARTED_AT: String(cycleStartedAt),
      PATTERN_SCAN_WIP_SCANNED: String(scanned),
      PATTERN_SCAN_WIP_RISING: JSON.stringify(results.risingLows),
      PATTERN_SCAN_WIP_DB: JSON.stringify(results.doubleBottom),
      PATTERN_SCAN_WIP_IHS: JSON.stringify(results.invHeadShoulders),
      PATTERN_SCAN_WIP_BOX: JSON.stringify(results.boxRangeLow)
    });
    scheduleRelay_('scanChartPatternsWorker_', RELAY_DELAY_MS);
    return;
  }

  props.setProperties({
    PATTERN_SCAN_META: JSON.stringify({ scannedAt: formatKstTime(Date.now()), universe: universe.length, scanned: scanned }),
    ...
    PATTERN_SCAN_CURSOR: '0'
  });
  props.deleteProperty('PATTERN_SCAN_CYCLE_STARTED_AT');
...
// 시간대를 띄워 6분 실행 제한을 피한다. 고정 일일 트리거(17:00 KST) 진입점 - 이어달리기 워커를 시작만 시킨다.
function scanPullback() {
  scanPullbackWorker_();
}

function scanPullbackWorker_() {
  clearRelayTriggers_('scanPullbackWorker_');

  var props = PropertiesService.getScriptProperties();
  var universe = fetchFullUniverse_();

  var cursor = parseInt(props.getProperty('PATTERN_PULLBACK_CURSOR') || '0', 10);
  var cycleStartedAt = parseInt(props.getProperty('PATTERN_PULLBACK_CYCLE_STARTED_AT') || '0', 10);
  var isNewCycle = cursor === 0 || !cycleStartedAt;
  if (isNewCycle) cycleStartedAt = Date.now();

  var matches = isNewCycle ? [] : JSON.parse(props.getProperty('PATTERN_PULLBACK_WIP_MATCHES') || '[]');
  var scanned = isNewCycle ? 0 : parseInt(props.getProperty('PATTERN_PULLBACK_WIP_SCANNED') || '0', 10);
  ...
  for (; i < universe.length; i++) {
    if (Date.now() - startedAt > PULLBACK_TIME_BUDGET_MS) break; // forceFinish여도 이번 호출 시간예산은 항상 지킨다
  ...
  if (i < universe.length && !forceFinish) {
    props.setProperties({
      PATTERN_PULLBACK_CURSOR: String(i),
      PATTERN_PULLBACK_CYCLE_STARTED_AT: String(cycleStartedAt),
      PATTERN_PULLBACK_WIP_SCANNED: String(scanned),
      PATTERN_PULLBACK_WIP_MATCHES: JSON.stringify(matches)
    });
    scheduleRelay_('scanPullbackWorker_', RELAY_DELAY_MS);
    return;
  }

  props.setProperties({
    PATTERN_SCAN_PULLBACK_META: JSON.stringify({ scannedAt: formatKstTime(Date.now()), universe: universe.length, scanned: scanned }),
    PATTERN_SCAN_PULLBACK: JSON.stringify(matches),
    PATTERN_PULLBACK_CURSOR: '0'
  });
  props.deleteProperty('PATTERN_PULLBACK_CYCLE_STARTED_AT');
  props.deleteProperty('PATTERN_PULLBACK_WIP_SCANNED');
  props.deleteProperty('PATTERN_PULLBACK_WIP_MATCHES');
...
var INVEST_SIGNAL_MAX_CYCLE_MS = RELAY_MAX_CYCLE_MS;
...
function scanInvestSignalWorker_() {
  clearRelayTriggers_('scanInvestSignalWorker_'); // 고정 킥오프 트리거(scanInvestSignal)는 다른 이름이라 안전
  ...
  if (i < universe.length && !forceFinish) {
    ...
    scheduleRelay_('scanInvestSignalWorker_', RELAY_DELAY_MS);
    return;
  }
