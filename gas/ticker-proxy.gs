...
var GOLD_MIN_DAYS_SINCE_CROSS = 3; // 전환 시점->랠리고점까지 최소 3거래일 지나야 반등 확인 가능
var GOLD_RECOVER_MIN = 0.05;     // 전환가 대비 랠리고점까지 최소 5% 반등해야 유효
...
// 골파기 후 추세 전환(MA60 기준): 종가가 한동안 60일 이평 아래(하락추세)에 있다가
// 이평을 위로 돌파(전환) -> 랠리(반등 고점) -> 눌림목(현재가) 구조.
...
function detectGoldPitReversal_(daily) {
  ...
  // 하락추세->상승추세 전환(종가가 MA60 아래에서 위로 올라선) 가장 최근 지점 탐색.
  ...
  // 전환 이후 랠리 고점(반등 최고점) 탐색
  var rallyPeakIdx = crossIdx;
  for (var m = crossIdx + 1; m < n; m++) if (win[m].high > win[rallyPeakIdx].high) rallyPeakIdx = m;
  if (rallyPeakIdx <= crossIdx) return null;
  if (rallyPeakIdx - crossIdx < GOLD_MIN_DAYS_SINCE_CROSS) return null;
  if (rallyPeakIdx >= n - 1) return null; // 랠리 고점이 마지막 봉이면 아직 눌림목이 나오기 전

  var crossPrice = win[crossIdx].close;
  var rallyPeak = win[rallyPeakIdx].high;
  var rallyPct = (rallyPeak - crossPrice) / crossPrice;
  if (rallyPct < GOLD_RECOVER_MIN) return null;

  // 눌림목: 현재가가 랠리 고점보다는 낮고(신고가 갱신 중이 아니고) 전환가보다는 높아야
  // (전환가 밑으로 되돌리면 추세전환 자체가 무효화된 것으로 판단)
  var lastClose = win[n - 1].close;
  if (lastClose >= rallyPeak) return null;
  if (lastClose < crossPrice) return null;

  var pullbackPct = (rallyPeak - lastClose) / rallyPeak;
  if (pullbackPct > GOLD_PULLBACK_MAX) return null; // 눌림이 너무 깊으면 재붕괴 우려로 제외

  // MA60이 지금도 우상향인지 재확인(진짜 추세전환인지 이중 확인)
  var ma10Ago = ma[Math.max(0, n - 11)];
  if (ma[n - 1] == null || ma10Ago == null || ma[n - 1] <= ma10Ago) return null;

  return {
    ma_period: period,
    cross: { date: win[crossIdx].date, price: crossPrice },
