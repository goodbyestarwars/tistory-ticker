...
var RECENCY_MAX_GAP = 10;        // 마지막 스윙/저점이 최근 10거래일 안에 있어야 "최근 봉 기준"으로 인정
...
function detectRisingLows_(daily) {
  ...
  return {
    ...
    signal: { date: win[lastLowIdx].date, price: lastLow },
  ...
  for (var a = 0; a < lowIdxs.length - 1; a++) {
    for (var b = a + 1; b < lowIdxs.length; b++) {
      ...
      return {
        ...
        signal: { date: win[i2].date, price: low2 },
    ...
    for (var b = a + 1; b < lowIdxs.length - 1; b++) {
      for (var c = b + 1; c < lowIdxs.length; c++) {
        ...
        return {
          ...
          signal: { date: win[iR].date, price: right },
