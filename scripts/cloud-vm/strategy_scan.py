...
#
# 2026-08-20(같은 날 추가 수정): 처음엔 무제한(NPS_TOP_N=None, DIVIDEND_TOP_N/
# ETF_RETURN_TOP_N과 같은 패턴)으로 뒀는데, 이건 국민연금 URL 버그(19차) 탓에 그때까지
# 실제로는 항상 0건이라 진짜 규모를 본 적이 없었다. 버그를 고치고 나니 유니버스 전체
# 대비 매칭 종목이 수백 건 단위로 나와서, js/strategy-search.js의 renderNpsTable()이
# 이걸 섹터 구분 없이 표 하나로 통째로 그리다 보니(다른 무제한 카테고리는 실제로는
# 수십 건 수준이라 문제가 없었음) "조건 자세히"(details 토글) 클릭 시 그 큰 표 전체가
# 리플로우되면서 브라우저가 멈추는 문제로 이어졌다(사용자 리포트). BLUECHIP_TOP_N/
# OPENING_GAP_TOP_N과 같은 패턴으로 상위 N개로 제한한다.
NPS_TOP_N = 100
NPS_METHODOLOGY_NOTE = (
    '국민연금공단이 공공데이터포털(data.go.kr)에 공개한 국내주식 투자정보 기준, 국민연금이 보유한 종목 중 '
    '보유 지분율(%) 상위 {top_n}개를 표시합니다. 이 데이터는 매일 갱신되지 않고 국민연금공단이 공시하는 시점의 '
    '스냅샷입니다(화면의 기준일 참고) - 그 이후 실제 보유 현황과 다를 수 있습니다. 종목명 표기가 달라 '
    '매칭되지 않는 종목은 목록에서 빠질 수 있습니다.'
).format(top_n=NPS_TOP_N)
