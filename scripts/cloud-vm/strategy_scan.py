...
# 2026-09-04: 조건 설명을 한 문단에서 "라벨 + 한 줄" 구조로 바꿨다. 목표주가 괴리 설명이
# 714자 8문장짜리 한 덩어리였고(다른 전략의 약 3배), 계산식·제외조건·보정·성격이 한
# 문단에 섞여 있어 "왜 이 종목이 여기 있지"를 확인하려면 처음부터 다 읽어야 했다
# (사용자 지적: "위 설명 글 너무 길지??").
#
# 라벨과 내용은 탭으로 나눈다 - 산문에 절대 안 나오는 문자라 프론트가 안전하게 쪼갤 수
# 있고, 탭이 없는 줄은 라벨 없는 문단으로 그대로 그려서 예전 형식(한 덩어리 문자열)도
# 그대로 표시된다(js/strategy-search.js renderMethodology).
def methodology(*rows):
    """(라벨, 내용) 줄들을 화면용 문자열로 합친다."""
    return '\n'.join('%s\t%s' % (label, text) for label, text in rows)


TARGET_PRICE_METHODOLOGY_NOTE = methodology(
    ('계산', '같은 업종(WICS 섹터) PER·PBR 중앙값을 적정 배수로 보되, 현재 배수에서 '
             '{reversion:.0f}%만큼만 좁혀진다고 봅니다(부분 재평가). 한 번에 다 수렴한다고 '
             '가정하지 않습니다. 평균 대신 중앙값을 쓰는 것은 이익이 0에 가까운 종목 하나가 '
             'PER을 튀게 만드는 것을 막기 위함입니다.'),
    ('제외', '적자 기업 · 업종 비교 표본 {min_peers}개 미만 · PER {per_cap:.0f}배/PBR '
             '{pbr_cap:.0f}배 초과(비교 표본에서 제외) · 계산된 괴리율 {max_gap:.0f}% 초과'),
    ('보정', 'KIS 애널리스트 목표가가 있고 자체 계산값이 더 높으면, 애널리스트 목표가까지의 '
             '거리 중 {analyst_anchor:.0f}%만 반영해 낮춥니다(나머지 {analyst_buffer:.0f}%는 '
             '데이터·시점 차이를 위한 여유). 애널리스트 목표가를 그대로 복사하지 않습니다.'),
    ('표시', '괴리 {min_gap:.0f}% 이상, 높은 순 최대 {top_n}종목. 후속 애널리스트 검증에서 '
             '기준 미달이 되면 빠질 수 있습니다.'),
    ('성격', '6~12개월 참고 목표주가입니다. 과거 적중률 백테스트가 끝난 확정 예측값이 아니라 '
             '일관된 계산식에 따른 근사치입니다.'),
...
METHODOLOGY_NOTE = methodology(
    ('계산', '펀더멘탈 점수(DART ROE·부채비율 기준) {min_score}점 이상 + 120일 이동평균 대비 '
             '{max_disp}% 이하로 가격이 눌린 종목'),
    ('제외', '최근 20일 평균 거래대금 10억원 미만 · 주가 1,000원 미만 · 서비스 테마 분류'),
    ('표시', '섹터별로 가장 많이 눌린 상위 {top_n}개'),
    ('성격', 'PER·PBR 같은 실제 밸류에이션 데이터가 없어 "장기 추세 대비 가격 눌림"으로 '
             '근사한 값입니다. 진짜 저평가(이익·자산 대비 싼 가격)와는 다를 수 있습니다.'),
...
DIVIDEND_METHODOLOGY_NOTE = methodology(
    ('계산', 'DART 사업보고서에 현금배당금이 공개된 국내 보통주를 넓게 표시합니다'
             '(네이버식 배당 랭킹 기준).'),
    ('정렬', '배당수익률 또는 주당 현금배당금'),
    ('제외', 'ETF·ETN·스팩·우선주·거래정지·정리매매·동전주 · 평균 거래대금 {turnover:.0f}억원 미만'),
    ('성격', '3년 순이익 증가·고배당률·최소 배당성향 조건은 적용하지 않는 단순 랭킹입니다. '
             '배당 데이터가 아직 DART 캐시에 없는 종목은 임의 추정하지 않고 다음 배치 수집 '
             '후 반영합니다.'),
).format(turnover=MIN_AVG_TURNOVER / 100000000)
...
NPS_TOP_N = None
NPS_METHODOLOGY_NOTE = methodology(
    ('정렬', '보유 지분율(%) 높은 순. 화면에서 지분율 기준을 바꿀 수 있습니다.'),
    ('출처', '국민연금기금운용본부(fund.nps.or.kr)가 공개한 국내주식 투자종목'),
    ('제외', '종목명 표기가 달라 매칭되지 않는 종목은 목록에서 빠질 수 있습니다.'),
    ('성격', '매일 갱신되지 않고 국민연금이 공시하는 시점의 스냅샷입니다. '
             '그 이후 실제 보유 현황과 다를 수 있습니다.'),
)
...
def main():
    ...
    output['categories']['etfReturn'] = {
        ...
        'methodology': methodology(
            ...
            ('정렬', '기본은 1개월 누적수익률. 화면에서 1·3·6·12개월을 선택해 다시 정렬할 수 '
                     '있으며, 각 종목의 네 기간 누적수익률을 함께 표시합니다.'),
