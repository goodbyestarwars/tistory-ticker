# -*- coding: utf-8 -*-
"""역헤드앤숄더(레퍼런스 기반 재설계, inv_head_shoulders_v2.py) 전종목 스캔 - "실제
확률"(백테스트 승률)을 확인하기 위한 수동 실행 전용 스크립트(타이머 없음,
rescan_patterns.py와 같은 성격). SQLite daily_prices만 읽고 키움 API는 재호출하지 않는다.

daily_scan_cache.json에 쓰지 않는다 - 기존 역헤드앤숄더(pattern_detect.
detect_inv_head_shoulders)는 이미 차트검색 화면에 나가고 있지만 이건 조건이 다른 재설계
버전이라, 화면에 붙이기로 확정된 게 아니라 별도 산출물에만 저장한다."""
...
OUTPUT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'inv_head_shoulders_v2_backtest.json')
BACKTEST_HOLD_DAYS = ihs.DEFAULT_HOLD_DAYS
BACKTEST_SLIPPAGE_PCT = ihs.DEFAULT_SLIPPAGE_PCT
MIN_ROWS = ihs.LOOKBACK_WINDOW + ihs.SWING + 5
...
def main():
    ...
    for i, code in enumerate(codes):
        ...
        df = ihs.compute_inv_head_shoulders_signal(code, conn=conn, rows=rows)
        if df.empty:
            continue
    ...
    summary = ihs.summarize_backtest(net_returns)
    ...
    print('\n=== 역헤드앤숄더(하락 추세선 돌파 + 거래량 죽은 눌림목) 백테스트 ===')
