...
/100으로 수정(_kis_row_to_common 참고).
...
# ---- KIS "시세" 버전(FHPTJ04030000, [0403]) - 실험적, 파라미터 의미 미확인 ----

def fetch_kis_time_today(token, appkey, appsecret):
    """[0404](일별)이 토스/키움HTS와 안 맞아서 시도하는 대안 - [0403](시세성)이 HTS의
    실시간 종합 수치와 더 가까운지 실측 확인용. market_iscd/sector_iscd 기본값(999/S001)은
    공식 예제 그대로라 "코스피 단독"이 맞는지 미검증 - raw 응답을 로그로 남겨 확인한다."""
    output = kis_client.fetch_market_investor_time(token, appkey, appsecret)
    if not output:
        return None
    row = output[0]
    logger.info('investor_trend KIS time(FHPTJ04030000) raw row: %s', json.dumps(row, ensure_ascii=False))

    def amt(key):
        try:
            return float(row.get(key) or 0) / 100  # 백만원 -> 억원 가정(FHPTJ04040000과 동일 관례로 추정)
        except (TypeError, ValueError):
            return 0.0
...
def refresh_recent_kis_time(appkey, appsecret):
    token = kis_client.get_token(appkey, appsecret)
    row = fetch_kis_time_today(token, appkey, appsecret)
    if not row:
        logger.warning('investor_trend: KIS time fetch got no row')
        return
    conn = db_schema.get_conn()
    try:
        db_schema.upsert_investor_trend_rows(conn, [row])
        logger.info('investor_trend KIS time recent refreshed: %s ind=%.0f frgn=%.0f orgn=%.0f',
...
_FORCE_KIS_REBACKFILL = True  # 2026-07-20: frgn 필드를 frgn_ntby_tr_pbmn->frgn_reg_ntby_pbmn으로
# 고쳤는데 이미 저장된 과거 행들은 옛 필드값 그대로라 재배포 1회는 강제로 다시 백필해야
# 전 구간에 고친 필드가 적용됨. 재배포 이후엔 다시 False로 되돌릴 것(매 재시작마다
# 130여회 호출은 낭비).
...
def _refresh_recent_chain(has_kiwoom, kiwoom_appkey, kiwoom_secretkey, has_kis, kis_appkey, kis_appsecret):
    """"오늘" 행 갱신 - 임시 진단 순서(2026-07-20 3차): KIS "시세"(FHPTJ04030000) 실험 ->
    KIS "일별"(FHPTJ04040000) -> 키움(ka10051) -> 네이버. 실험용 시세 TR을 검증하는 동안만
    이 순서를 쓰고, 결론 나면 정리할 것."""
    if has_kis:
        try:
            refresh_recent_kis_time(kis_appkey, kis_appsecret)
            return
        except Exception:
            logger.exception('refresh_recent_kis_time failed - falling back for this tick')
        try:
