...
run_news_momentum_if_due() {
  ...
  # 2026-09-14 사용자 승인: 이 배치는 전 상장종목(3,913)을 20분 슬라이스로 이어 도는데, 예전엔
  # 배포 잠금(fd 200)을 쥔 채 전면에서 돌아 하루 대부분 배포가 최대 20분씩 밀렸다(같은 날 VM
  # fuser로 확인한 잠금 보유자: deploy_check.sh -> flock -> news_momentum_scan.py). 배포와 무관한
  # 수집이라 백그라운드로 떼고 fd 200은 닫는다. 겹침은 원래 쓰던 전용 잠금($MOMENTUM_LOCK,
  # 실행 중이면 종료코드 75로 즉시 빠짐)이 그대로 막는다. 날짜 마커·검증·로그 문구는 그대로다.
  (
...
run_price_recap_cleanup_once() {
  ...
  # 2026-09-14: 뉴스 모멘텀 배치가 백그라운드로 옮겨가 같은 news_momentum.db를 동시에 쓸 수 있어
  # 배치 전용 잠금을 잡고 실행한다. 배치가 도는 중이면 잠금을 못 잡아 아래 실패 분기로 가고
  # 마커가 남지 않으니 다음 5분 회차가 다시 시도한다.
  if flock -n "$MOMENTUM_LOCK" "$PYTHON" "$APP_DIR/cleanup_price_recap_topics.py" \
