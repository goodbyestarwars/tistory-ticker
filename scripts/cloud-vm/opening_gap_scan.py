# -*- coding: utf-8 -*-
"""시초 갭상승(opening_gap.py) 전종목 스캔 - "실제 확률"(백테스트 승률)을 확인하기 위한
수동 실행 전용 스크립트(타이머 없음, rescan_patterns.py와 같은 성격). SQLite
daily_prices만 읽고 키움 API는 재호출하지 않는다.

daily_scan_cache.json에 쓰지 않는다 - 기존 시초 갭상승(pattern_detect.detect_opening_gap)은
...
OUTPUT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'opening_gap_backtest.json')
BACKTEST_HOLD_DAYS = og.DEFAULT_HOLD_DAYS
BACKTEST_SLIPPAGE_PCT = og.DEFAULT_SLIPPAGE_PCT


def log(msg):
    print('[opening_gap_scan] ' + msg, flush=True)
...
def main():
    ...
    for i, code in enumerate(codes):
        ...
        df = og.compute_opening_gap_signal(code, conn=conn, rows=rows)
        if df.empty:
            continue

        net_returns.extend(og.backtest_entry_signal(df, hold_days=BACKTEST_HOLD_DAYS, slippage_pct=BACKTEST_SLIPPAGE_PCT))
    ...
    summary = og.summarize_backtest(net_returns)
