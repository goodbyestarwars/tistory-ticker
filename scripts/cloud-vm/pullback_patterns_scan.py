...
def main():
    ...
    for i, code in enumerate(codes):
        ...
        if len(rows) >= pp.BLUECHIP_MIN_DAYS:
            ...
            bluechip_returns.extend(pp.backtest_entry_signal(bluechip_df, hold_days=BACKTEST_HOLD_DAYS, slippage_pct=BACKTEST_SLIPPAGE_PCT))

        surge_df = pp.compute_surge_pullback_signal(code, conn=conn, rows=rows)
        surge_returns.extend(pp.backtest_entry_signal(surge_df, hold_days=BACKTEST_HOLD_DAYS, slippage_pct=BACKTEST_SLIPPAGE_PCT))
