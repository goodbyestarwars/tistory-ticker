...
# 2026-07-19(5차): 처음엔 인메모리 dict로 짰는데, kiwoom-deploy.timer가 git push마다
# 서비스를 재시작해서(개발 중엔 하루에도 여러 번) 캐시가 계속 날아가는 문제를 사용자가
# 바로 발견(재배포 직후 재현 테스트 중) - db_schema.kis_flow_cache(SQLite)로 옮겨서
# 재시작에도 살아남게 함.
...
def _live_investor_row_from(rows, end_dt):
    ...
    같은 패턴(외국인·기관은 0이 아닌데 개인만 0)이 서로 다른 두 종목에서 반복 관측된 점을
    보고 처음엔 "실측 정황 증거"로만 판단했는데, 이후 KRX 투자자별 매매동향 공식 집계
    스케줄을 확인해 구조적 근거를 찾았다(키움 HTS 도움말 [0796] 종목별투자자,
    download.kiwoom.com/hero4_help_new/0796.htm) - 당일 잠정정보 집계시각은 09:30(외국인만
    제공)·10:00·11:30·13:20·14:30이고, 확정정보는 KRX 15:35(NXT 20:05)에 나온다. 외국인·
    기관은 거래소에 등록된 계좌라 체결 건별로 집계 가능해 이 잠정 스케줄보다 촘촘하게
    갱신되는 반면, 개인은 등록 태그가 없어 "전체 - 외국인 - 기관 - 기타법인" 잔차로만
    산출되므로 거래소 공식 집계 이전에는 계산 자체가 불가능하다. 그래서 이 실시간 패치에서는
    개인 순매매를 아예 신뢰하지 않기로 한다 - ind_invsr 값은 쓰지 않고 항상 None으로 둬
    (호출부가 '당일' 행의 ind_net을 None으로 채움) 프론트가 기존 규칙대로 "-"로 표시하게
    한다(개인 열이 구조적으로 없는 네이버 폴백과 동일한 처리 - 원본을 신뢰할 수 없는 값은
    0으로 채우지 않는다). 외국인·기관은 등록 계좌 기반이라 계속 신뢰한다. 개인의 확정치는
    KIS 일별 TR이 열리는 15:40(KST) 이후 다음 조회부터 정상 반영된다."""
    if not rows:
        return None
...
def _daily_rows_from_kis_with_fallback_cache(kis_appkey, kis_appsecret, code, end_dt, frgn_by_date, target_days):
    ...
    db_schema.kis_flow_cache(SQLite)에 저장 - 인메모리였다면 kiwoom-deploy.timer의
    git push마다 재시작으로 매번 날아갔을 것(실측으로 발견됨)."""
    ...
    try:
        try:
            ...
            cached_rows, updated_at = db_schema.load_kis_flow_cache(conn, code, target_days)
            if cached_rows is None:
                raise
            age_sec = (datetime.now(timezone.utc) - datetime.fromisoformat(updated_at)).total_seconds()
