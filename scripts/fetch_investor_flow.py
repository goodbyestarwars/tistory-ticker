...
사용법 (택1):
  1) 저장소 루트에 .env 파일 만들고(.gitignore 대상이라 git에 안 올라감) 아래처럼 채운 뒤 그냥 실행:
       KIWOOM_APPKEY=xxx
       KIWOOM_SECRETKEY=yyy
     python scripts/fetch_investor_flow.py
  2) 환경변수로 직접 전달:
...
주의:
- 앱키/시크릿은 반드시 .env 파일 또는 환경변수로 전달할 것. 이 스크립트 어디에도 하드코딩하지 말 것
...
def load_dotenv():
    """REPO_ROOT/.env(있으면)의 KEY=VALUE 줄을 os.environ에 채운다(이미 설정된 실제 환경변수는
    덮어쓰지 않음). setx로 시스템 환경변수를 등록하면 새 터미널이 필요해 매번 헷갈리길래,
    별도 패키지 없이 이 파일 하나로 대체할 수 있게 함 - .env는 .gitignore 대상."""
...
def main():
    ...
    if not appkey or not secretkey:
        log('KIWOOM_APPKEY / KIWOOM_SECRETKEY 환경변수가 필요합니다 (.env 파일 또는 환경변수로 설정).')
        sys.exit(1)
