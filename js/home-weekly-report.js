...
(function (global) {
  ...
  function render(root, payload) {
    ...
    root.innerHTML = '<div class="hwr-head"><div class="hwr-head-copy"><span class="hwr-eyebrow">WEEKEND BRIEF</span><h2>' + title + '</h2><p>' + subtitle + '</p></div>' + sentimentArt(indices) + '<div class="hwr-period">' + escapeHtml(data.week && data.week.label || '기준일 확인 중') + '<small>금요일 장 마감 기준</small></div></div>'
      ...
      + '<section class="hwr-stock-section hwr-candidate-section"><div class="hwr-section-heading"><strong>다음 주 후보</strong><span>예측이 아니라 겹친 선행 신호 기준</span></div><div class="hwr-columns"><article><div class="hwr-card-title"><strong class="is-up">뜨거워질 후보</strong><span>거래·수급 선행 신호</span></div>' + stockListWithReasons(data.hotCandidates && data.hotCandidates.domestic, 'domestic') + '</article><article><div class="hwr-card-title"><strong class="is-up">미국</strong><span>거래·수급 선행 신호</span></div>' + stockListWithReasons(data.hotCandidates && data.hotCandidates.us, 'us') + '</article></div><div class="hwr-columns"><article><div class="hwr-card-title"><strong class="is-down">차가워질 후보</strong><span>약세·유동성 신호</span></div>' + stockListWithReasons(data.coldCandidates && data.coldCandidates.domestic, 'domestic') + '</article><article><div class="hwr-card-title"><strong class="is-down">미국</strong><span>약세·유동성 신호</span></div>' + stockListWithReasons(data.coldCandidates && data.coldCandidates.us, 'us') + '</article></div></section>'
