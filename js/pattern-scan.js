...
(function (global) {
  ...
  var TABS = [
    ...
    { key: 'volumeBreakout', label: '거래량 돌파(10분)', desc: '개장 10분 시점(09:10)의 당일 누적 거래량이 전일 하루치 거래량 이상이고, 시가가 전일종가보다 높게(갭상승) 출발한 종목입니다. 거래가 갑자기 몰린 자리를 찾는 조건이며, 갭상승 조건 때문에 하락 출발 종목은 거래량이 아무리 몰려도 빠집니다. 당일 거래량·거래대금·거래증가율 상위 목록을 후보로 쓰므로 순위 밖으로 밀린 종목은 잡히지 않습니다.' }
  ];
  ...
  function scannerInterpretation(item, patternKey) {
    ...
    return {
      ...
      volumeBreakout: '갭상승 출발 + 개장 10분 누적 거래량이 전일 하루치 이상'
