...
(function (global) {
  ...
  var TABS = [
    ...
    { key: 'maCloudBreakout', label: '이평 상승 초입형', desc: '최소 250봉 데이터에서 종가가 224일선 ±3% 이내이고, 현재 종가가 일목 구름 상단을 아직 넘지 않았으며 고가가 구름 상단 3% 이내로 접근한 종목입니다(구름 하단 아래에서 올라오는 중이어도 포함).' },
  ...
  function scannerSignal(item, patternKey) {
    ...
    if (patternKey === 'maCloudBreakout') return '224일선 근접·구름 상단 시도';
