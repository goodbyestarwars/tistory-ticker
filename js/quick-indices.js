...
(function (global) {
  ...
  // 코스피/코스닥은 KRX 정규장(평일 09:00~15:30 KST), 코스피 야간선물은 평일
  // 18:00~익일 06:00, 나머지(해외선물/원자재/환율/코인)는 사실상 24시간에 가까워
  // 주말만 장마감으로 취급한다. 정확한 거래소 캘린더(공휴일 등)는 반영하지 않은
  // 근사치라 실제와 몇 분~하루 단위로 어긋날 수 있음을 감안할 것.
  //
  // 2026-08-05: js/kospi-futures.js에서 발견·정정한 두 가지를 여기도 통일했다(사용자 요청).
  // (1) 마감 시각 05:00 -> 06:00 - 05:20에 실제로 야간선물이 거래되는 걸 실시간으로
  // 확인해 기존 05:00 주석·값 자체가 부정확했던 것으로 판명(공식 문서 대조는 못 했으나
  // 실시간 관찰이 오래된 주석보다 신뢰도가 높다고 판단). (2) 요일 미고려 버그 - 기존엔
  // mins만 보고 판정해 토요일 저녁·일요일 새벽처럼 실제로는 세션이 없는 구간도 "실시간"으로
  // 잘못 표시했다. 야간선물은 "평일 저녁에 열려 다음날 새벽에 닫히는" 세션이라 시작(그날이
  // 평일 저녁)과 종료(전날이 평일이었던 새벽, 즉 오늘이 화~토)를 따로 확인해야 토요일 밤
  // (금요일 세션의 정상 연장)은 열림으로, 일요일 새벽(토요일엔 세션이 없었으므로)은 닫힘으로
  // 구분된다 - js/kospi-futures.js의 isMarketOpen과 동일 로직.
