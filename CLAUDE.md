...
- `.github/workflows/page-speed.yml`: 라이브 페이지의 **체감 로딩 속도**를 헤드리스 크롬으로
  잰다(FCP·LCP + 페이지별 "목록·수치가 실제로 채워진 시각"). API 응답시간만으로는 화면이
  언제 쓸 만해지는지 알 수 없어서 별도로 둔다. 러너 위치·회선이 실제 사용자와 다르므로
  절대값보다 **페이지 간 비교와 변경 전후 비교**에 쓴다. 수동 실행 전용.
