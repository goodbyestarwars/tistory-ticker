...
| `js/short-pressure.js`, `css/short-pressure.css` | **공매도 압박 위젯 - 2026-07-11 보류(메뉴에서 내림, 코드는 유지).** 네이버 `short_trade.naver` 탭도 실제로는 KRX 페이지(`data.krx.co.kr/comm/srt/srtLoader/index.cmd`)를 iframe으로 감싼 것뿐이라 실데이터가 없고, 그 KRX 로더도 결국 로그인 필요한 `getJsonData.cmd`를 호출하는 것으로 보여 무료 우회 경로가 없음(`?debugShortNaver=1&code=`로 직접 확인). 데이터 소스가 새로 생기면 `getShortPressure`/`fetchShortTradeRows_`를 고쳐서 재사용. |
