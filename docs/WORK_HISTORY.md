...
**2026-08-28 증시온도 점수 산식·출처 공개**: 내부 120점 가중합을 바꾸지 않되, 화면에는 항상 100점 환산 점수와 온도 구간을 함께 보여주도록 했다. `시장 구성 요소`에는 항목별 현재 구간을 표시하고, 접을 수 있는 계산 기준·데이터 출처 목록을 추가했다. 사용자는 이제 예를 들어 24.1℃를 60.3/100점의 중립 구간으로 읽고, 각 항목이 몇 점 만점이며 어떤 임계값과 원자료로 계산됐는지 확인할 수 있다. GitHub Pages 자동 배포 대상이며 UI 회귀 검사와 로컬 화면 검증을 완료했다.
