...
모두 `US:INTC`로 연결하고 화면에는 `인텔`을 우선 표시하도록 공통 검색창과 미국 모듈의
표시명 사전을 통일했다. 미국 API가 영어 회사명을 반환해도 주요 종목은 한글명으로 표시하며,
거래소명·시장 상태·재무 금액 단위도 한국어로 변환한다. 내부 코드와 원문 뉴스 링크는 유지한다.
