...
**2026-08-28 증시온도 빚투 위험도 참고 지표 복원**: 최초 KOFIA 연결의 의도대로 신용융자·예탁금·반대매매 기반 `빚투 위험도`는 상태와 수치만 보여주는 참고 지표로 되돌렸다. 온도 합산 배점과 영향도 순위·점수 막대에서는 제외하고, 실제 온도는 기존 9개 시장 지표의 110점 만점을 40℃로 환산한다. KOFIA 데이터 조회와 안정·주의·과열 판정은 유지한다. GAS 캐시 키를 `market_temp_v9`로 올려 이전 120점 산식 응답을 재사용하지 않으며, GitHub Pages·GAS 자동 배포 대상이다.
