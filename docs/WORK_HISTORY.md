...
**2026-08-25 미국 의회 거래 공시 시그널 추가**: 미국 종목분석에 Quiver Congress Trading API 기반의 별도 공시 패널을 추가했다. 서버 전용 `QUIVER_API_KEY`와 3시간 SQLite 캐시를 사용하며, 의원·거래 방향·금액구간·실제 거래일·신고일·신고 지연일수를 표시한다. 최대 45일 지연 가능성, 배우자·부양가족 명의 가능성, 복사매매가 아니라는 안내와 원문 링크를 고정해 제공자 데이터가 없을 때도 빈 화면 대신 연동 상태를 명확히 보여준다. `test_congress_trading.py` 및 UI 계약 테스트를 추가했다.
