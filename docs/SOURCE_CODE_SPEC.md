...
| `gongpasan_strategy.py` | 227 | 공파산(역매공파 - 역배열·매집봉·공구리·파란점선/오돌이) 단일 종목 신호 계산(`.claude/skills/synced/yeokmaegongpa` 스킬 정의 그대로 이식) + 백테스트(2026-08-20, pandas/numpy) | `calculate_gongpasan_signal`, `backtest_gongpasan`, `summarize_backtest` |
| `gongpasan_scan.py` | 185 | "공파산 타점" 탭 전종목 스캔(SQLite만 읽음, 키움 재호출 없음) + 백테스트 집계, `daily_scan_cache.json`에 patternScan.patterns.gongpasan/gongpasanBacktest로 저장(angle_momentum_scan.py와 독립 - 서로 참조 안 함) | `main` |
