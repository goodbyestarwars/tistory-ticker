...
KOFIA 보조지표를 사용하려면 VM `.env`에 `DATA_GO_KR_KOFIA_SERVICE_KEY`를 설정합니다. `/kofia-market`이 신용융자 잔고·투자자예탁금·반대매매 비중의 최근 추이를 제공하며, 시장온도 응답의 `kofia` 필드와 마지막 브리핑 카드에 연결됩니다. 온도 점수에는 합산하지 않습니다.
