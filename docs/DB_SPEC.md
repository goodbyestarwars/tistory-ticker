...
2026-09-15부터 `volume_profile_collector.py`(FastAPI 스레드)가 KRX 거래일 18:10~20:00 KST 하루 1회 종목판 캐시의 국내 순위 종목 + 최근 30일 저장 종목(`list_volume_profile_codes`, 최대 120)을 같은 형식으로 저장한다 - 이 종목들은 거래일마다 빠짐없이 쌓인다. 크기 가늠: 종목·날짜당 가격 수십~수백 행이라 120종목 × 200일 보존이면 수백만 행 수준. 종목분석·MY 화면은 같은 날부터 이 테이블 대신 최근 120거래일 일봉 추정치를 쓴다(실제 체결가 전환은 날수가 쌓인 뒤 검토).
